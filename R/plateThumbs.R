...
plateThumbs <- function(project_dir, plates = "all") {
  ...
  file_df <- tibble(name = filtered_file_list) %>%
  ...
  for(i in unique(join_df$plate)) {
    ...
    pw = (img_w*12)/300
    ph = (img_h*8)/300
    ggsave(file = glue::glue("{project_dir}/raw_plate_thumbs/{proj}_{unique(uniq_plate$plate)}.png"), uniq_plate_grid, dpi = 300, height = ph, width = pw)
  }
  message("Done")
}
