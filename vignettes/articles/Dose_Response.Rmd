...
```{r}
# Add variables that describe the PATH to processed images and desired well labels
cm <- cf %>%
...
# IMPORTANT! set the seed to ensure the 8 random wells are found in the example images
set.seed(99)
...
vo1 <- easyXpress::viewOverlay(data = o2,
                        ...
                        file = paste0(ex.dir, "eXDR/viewOverlay/overlay1.png")) # save to example dir
```
...
tf <- easyXpress::titerWF(data = raw.wells, Metadata_Experiment, bleach, strain, drug, doseR = T)
...
cb <- checkBalance(data = fw, drug, concentration_um,
                   design = dr$design %>%
                     dplyr::mutate(assay_bleach = paste(Metadata_Experiment, bleach)),
                   x = assay_bleach)
...
ce1 <- easyXpress::checkEff(data = drop, drug, strain, x = concentration_um, y = median_wormlength_um, fill = assay_bleach, scales = "free_x")
...
```{r, fig.align = 'center'}
# Regress the effect of the remaining independent bleaches for each drug using regEff()
reg <- easyXpress::regEff(data = drop, drug, d.var = median_wormlength_um, c.var = assay_bleach)
